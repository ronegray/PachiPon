...
def listener(listen_target: TARGET_DEVICE = "pad") -> tuple[int, int]:
    ...
    if listen_target == "pad":
        ...
        for keycode in range(px.GAMEPAD1_AXIS_TRIGGERLEFT, px.GAMEPAD1_AXIS_RIGHTY + 1):
            input_val = px.btnv(keycode)
            if abs(input_val) > _ANALOG_THRESHOLD_TRIGGER:
                return keycode, 1
        ...
        for keycode in _ASSIGNABLE_KEYS:
            if px.btnp(keycode):
                return keycode, 1
...
def _wrap_analog_input(
    key_code: int,
    input_sign: int = 1,
    hold_sec: float = _WAIT_SEC_KEYREPEAT,
    repeat_sec: float = _INTERVAL_SEC_KEYREPEAT,
) -> InputHandler:
    ...
    if key_code in (
        ...
        threshold = _ANALOG_THRESHOLD_XY
    ...
    # それ以外の全てのデジタルキー・ボタンの場合
        ...
        return lambda mode="once": (
            ...
            if mode == "once"
                ...
                key_code,
                hold=_sec_to_frames(_WAIT_SEC_KEYREPEAT),
                repeat=_sec_to_frames(_INTERVAL_SEC_KEYREPEAT),
            )
            if mode == "hold"
            else False
        )
