...
class PlayerSprite(BaseSprite):
    def __init__(
        ...
        self.img: px.Image = px.Image(w * 8, h)
        self.set_sprite_image(img_index)
        super().__init__(x, y, self.img, u, v, w, h, colkey)
    ...
    def set_sprite_image(self, image_type: PlayerSpriteType = PlayerSpriteType.HERO):
        chara_image = px.Image.from_image(AssetMap.get_assetpath(AssetID.IMAGE_CHARA))
        self.img.blt(0, 0, chara_image, 0, image_type * 16, 128, 16)
    ...
    def draw(self, x: int | None = None, y: int | None = None):
        ...
        if self.colkey is not None:
            ...
            px.blt(draw_x, draw_y, self.img, u, v, self.w, self.h)
